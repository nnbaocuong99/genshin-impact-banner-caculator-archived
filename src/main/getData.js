...
const i18n = require('./language_draft1')
...
const detectGameLocale = async (userPath) => {
  ...
  try {
    await fs.access(path.join(userPath, '/AppData/LocalLow/miHoYo/', 'users/output_log.txt'), fs.constants.F_OK)
    list.push('user')
  } catch (e) {}
  try {
